...
# Tests (under test/, mirroring lib/ structure — pure logic only, no Firebase)
flutter test
...
**Pure logic gets extracted to its own module next to `index.ts`, with a matching `*.test.ts`** — `ai_budget`, `ai_question_bank`, `ai_topic_similarity`, `async_pvp_turns`, `daily_challenge_dates`, `fixed_level_slicing`, `life_costs`, `quiz_answers`. The point of the split is testability: those modules import nothing from firebase-admin, so `npm test` (`node --test` over the compiled `lib/`) can exercise them without Firestore or a clock. When adding logic that is decidable from its arguments alone, put it in one of these (or a new one) rather than inline in `index.ts` — anything inside `index.ts` is effectively untested.
...
**Async PvP is Cloud-Function-only end to end** — `createAsyncPvpMatch` (picks the questions and the match parameters), `openAsyncPvpTurn` / `submitAsyncPvpAnswer` (stamp each question's deadline server-side and judge answers against it, decided in `async_pvp_turns`), `finishAsyncPvpMatch` (scores the round), then `finalizeAsyncPvpMatch` settles it. `firestore.rules` leaves the client nothing on `async_matches` but reading and declining, so anything new in that flow goes through a callable, not a client write: rules can check a document's shape but not where its contents came from, and every hole here was a client writing its own questions, answers or clock. Live matches (`/matches`) still write from the client and don't have this shape yet.
